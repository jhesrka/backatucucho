"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.FreePostTrackerService = void 0;
const data_1 = require("../../../data");
const domain_1 = require("../../../domain");
class FreePostTrackerService {
    getOrCreateTracker(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            const currentMonth = new Date();
            currentMonth.setDate(1); // Primer día del mes
            currentMonth.setHours(0, 0, 0, 0);
            // Usa findOneBy en lugar de findOne
            let tracker = yield data_1.FreePostTracker.findOneBy({
                user: { id: userId },
                monthYear: currentMonth
            });
            if (!tracker) {
                const user = yield data_1.User.findOneBy({ id: userId });
                if (!user) {
                    throw domain_1.CustomError.notFound("Usuario no encontrado");
                }
                tracker = new data_1.FreePostTracker();
                tracker.user = user;
                tracker.monthYear = currentMonth;
                tracker.count = 0;
                yield tracker.save();
            }
            return tracker;
        });
    }
}
exports.FreePostTrackerService = FreePostTrackerService;
