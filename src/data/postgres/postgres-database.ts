...
export class PostgresDatabase {
  ...
  async connect() {
    try {
      ...
      await this.datasource.query(`ALTER TABLE "producto_pedido" DROP CONSTRAINT IF EXISTS "FK_producto_pedido_producto";`).catch(err => console.warn("Could not drop FK_producto_pedido_producto:", err.message));
      
      // Also try dropping any other constraint on producto_pedido -> producto if it has a random name generated by TypeORM
      await this.datasource.query(`
        DO $FK_PP$
        ...
        BEGIN
            ...
            IF fk_pp_name IS NOT NULL THEN EXECUTE 'ALTER TABLE "producto_pedido" DROP CONSTRAINT IF EXISTS ' || quote_ident(fk_pp_name); END IF;
        END $FK_PP$;
      `).catch(err => console.warn("Could not drop dynamic FK:", err.message));

      await this.datasource.query(`ALTER TABLE "producto_pedido" ADD CONSTRAINT "FK_producto_pedido_producto" FOREIGN KEY ("productoId") REFERENCES "producto"("id") ON DELETE SET NULL;`).catch(err => console.warn("Could not add FK_producto_pedido_producto:", err.message));
